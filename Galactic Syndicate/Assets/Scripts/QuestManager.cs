...
public class QuestManager : MonoBehaviour
{
    ...
    public async Task TryCompleteQuests(Planet destinationPlanet)
    {
        ...
        foreach (var quest in activeQuests.ToList())
        {
            if (quest.OriginPlanet == destinationPlanet)
            {
                if (InventoryManager.instance.HasItem(quest.TargetItem, quest.RequiredQuantity))
                {
                    ...
                    // --- ANALYTICS OLAYI ---
                    AnalyticsManager.instance?.LogEvent("quest_completed", new Dictionary<string, object>
                    {
                        { "quest_item", quest.TargetItem.itemName },
                        { "quest_planet", quest.OriginPlanet.name },
                        { "reward", quest.Reward }
                    });
                    // -------------------------
                }
        ...
        if (completedQuests.Count > 0)
        {
            ...
            await SaveManager.instance.RequestSave();
        }
    }
